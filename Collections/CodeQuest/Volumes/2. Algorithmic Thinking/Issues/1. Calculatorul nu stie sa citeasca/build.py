...
def collect_doctor_findings(target: IssueTarget) -> list[Finding]:
    ...
    has_usable_latexmk = latexmk_available and (os.name != "nt" or perl_available)
    fallback_drivers: list[str] = []
    if texify_available:
        fallback_drivers.append("texify")
    if tectonic_available:
        fallback_drivers.append("tectonic")
    if pdflatex_available:
        fallback_drivers.append("pdflatex")

    if not has_usable_latexmk and not fallback_drivers:
        findings.append(
            Finding(
                "ERROR",
                "No usable TeX build driver is available. Install one of: latexmk (with Perl on Windows), texify, tectonic, or pdflatex.",
            )
        )
    elif latexmk_available and os.name == "nt" and not perl_available and fallback_drivers:
        findings.append(
            Finding(
                "WARN",
                f"Perl is not available, so builds will fall back from latexmk to {', '.join(fallback_drivers)} on this machine.",
            )
        )
    elif latexmk_available and os.name == "nt" and not perl_available and not fallback_drivers:
        findings.append(
            Finding(
                "ERROR",
                "latexmk is installed but Perl is unavailable, and no fallback driver (texify/tectonic/pdflatex) is present.",
            )
        )
...
def clean_target(target: IssueTarget) -> None:
    if target.build_dir.exists():
        shutil.rmtree(target.build_dir, onexc=remove_readonly)
...
def run_build_command(driver: str, command: list[str], cwd: Path) -> subprocess.CompletedProcess[str]:
    if driver != "pdflatex":
...
def build_target(target: IssueTarget) -> int:
    ...
    result = run_build_command(driver, command, staged_src_dir)
