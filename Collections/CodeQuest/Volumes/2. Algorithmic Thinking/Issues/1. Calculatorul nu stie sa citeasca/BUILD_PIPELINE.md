...
The driver prefers `latexmk` when available and usable. If that is not possible, it falls back in this order:

1. `texify`
2. `tectonic`
3. `pdflatex` (run twice to settle references)

This means a missing Perl installation on Windows no longer blocks builds if any fallback driver is available.
